...
export function DashboardOverview({ heartbeatStats, uptimeStats }: OverviewProps) {
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="bg-bg-surface border border-border-card rounded-[2rem] p-8">
...
function HealthCard({ label, value, description, icon: Icon, color, pulse }: any) {
  return (
    <div className="bg-bg-surface border border-border-card rounded-3xl p-8 group hover:border-border-card transition-all">
      <div className="flex items-center justify-between mb-6">
        <div className={cn("p-3 rounded-2xl bg-bg-subtle", color)}>
          <Icon className="w-6 h-6" />
        </div>
        {pulse && <div className="w-3 h-3 rounded-full bg-brand-error animate-ping" />}
      </div>
      <h4 className="text-4xl font-bold text-text-primary mb-2 tracking-tight">{value}</h4>
...
function FeatureCard({ title, description, stats, href, icon: Icon, buttonText }: any) {
  return (
    <div className="bg-bg-surface border border-border-card rounded-[2.5rem] p-10 flex flex-col h-full group hover:border-brand-primary/30 transition-all relative overflow-hidden">
      <div className="relative z-10">
        ...
        <h3 className="text-2xl font-bold text-text-primary mb-4 tracking-tight">{title}</h3>
        ...
        <div className="grid grid-cols-2 gap-6 mb-12">
          {stats.map((stat: any, i: number) => (
            <div key={i} className="bg-bg-subtle rounded-2xl p-4 border border-border-card">
        ...
        <Link 
          href={href}
          className="mt-auto w-full h-14 bg-bg-subtle hover:bg-brand-primary text-text-primary hover:text-white rounded-2xl font-bold transition-all flex items-center justify-center gap-3 group/btn"
        >
          {buttonText}
...
function ActivityItem({ title, time, type, description }: any) {
  ...
  return (
    <div className="flex items-start gap-4 p-4 rounded-2xl hover:bg-bg-subtle transition-colors border border-transparent hover:border-border-card">
